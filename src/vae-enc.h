...
// Architecture: conv1(2->64,k=7) -> 6x(3xresunit+snake+strided_conv) -> snake+conv2(2048->128,k=3)
// Output 128ch = mean[64] + scale[64]. Deterministic encode returns mean.
// Downsample: 2x2x4x4x5x6 = 1920x (inverts the decoder upsample frame for frame)
...
struct VAEEncoder {
    struct ggml_tensor *c1w, *c1b;  // conv1 [7, 2, 64], bias [64]
    VAEEncBlock         blk[6];
...
static void vae_enc_load(VAEEncoder * m, const char * path) {
    ...
    //   conv1: 2 -> 64
    //   block: [64->64, 64->128, 128->256, 256->512, 512->1024, 1024->2048]
    //   conv2: 2048 -> 128 (split: mean[64] + scale[64])
    // ResUnits run at in_ch (before downsample), unlike decoder (at out_ch, after upsample).
    static const int in_ch[]     = { 64, 64, 128, 256, 512, 1024 };
    static const int out_ch[]    = { 64, 128, 256, 512, 1024, 2048 };
    static const int strides[]   = { 2, 2, 4, 4, 5, 6 };
    ...
    m->c1w = ggml_new_tensor_3d(ctx, GGML_TYPE_F16, 7, 2, 64);
    m->c1b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 64);
    ...
    fprintf(stderr, "[VAE-Enc] Loaded: 6 blocks, downsample=1920x, F32 activations\n");
    gf_close(&gf);
}
...
static struct ggml_tensor * vae_enc_build_graph(struct ggml_context * ctx,
    ...
    // 6 encoder blocks: resunits(in_ch) -> snake(in_ch) -> strided conv(in_ch -> out_ch)
    for (int i = 0; i < 6; i++) {
        VAEEncBlock & b = m->blk[i];
        for (int r = 0; r < 3; r++) {
...
// Latent frames produced from T_audio samples, PyTorch Conv1d length rule per block
static int vae_enc_frames(int T_audio) {
    static const int strides[] = { 2, 2, 4, 4, 5, 6 };
    int              L         = T_audio;
    for (int i = 0; i < 6; i++) {
        int s = strides[i];
...
// Tiled encode: exact halo crop, mirror of the decoder tiling.
// A window of W*1920 samples yields exactly W latent frames, so each tile
// encodes the audio of [left, right) frames and keeps its core frames, every
// one of them carrying the full receptive field of the untiled encoder.
// Returns T_latent or -1 on error.
static int vae_enc_encode_tiled(VAEEncoder *  m,
                                ...
                                int           halo_frames = 16) {
    int T_latent = vae_enc_frames(T_audio);
    if (T_latent < 1) {
        fprintf(stderr, "[VAE-Enc] T_audio %d is shorter than one latent frame\n", T_audio);
        return -1;
    }
    if (T_latent > max_T_latent) {
        fprintf(stderr, "[VAE-Enc] T_latent %d exceeds max %d\n", T_latent, max_T_latent);
    ...
    fprintf(stderr, "[VAE-Enc] Tiled encode: %d tiles (core=%d, halo=%d)\n", num_tiles, core_frames, halo_frames);
    ...
    for (int i = 0; i < num_tiles; i++) {
        ...
        int win_start = left * 1920;
        int win_len   = (right == T_latent) ? (T_audio - win_start) : (right - left) * 1920;

        int tile_T = vae_enc_compute(m, audio + (size_t) win_start * 2, win_len);
        if (tile_T < 0) {
            fprintf(stderr, "[VAE-Enc] FATAL: tile %d encode failed\n", i);
            return -1;
        }
        if (tile_T < end - left) {
            fprintf(stderr, "[VAE-Enc] FATAL: tile %d does not cover its core\n", i);
            return -1;
        }

        // Tile output is [ne0=tile_T, ne1=128], channel contiguous, mean = ch 0..63
        std::vector<float> raw((size_t) 128 * tile_T);
        ggml_backend_tensor_get(m->graph_output, raw.data(), 0, raw.size() * sizeof(float));

        for (int t = start; t < end; t++) {
            for (int c = 0; c < 64; c++) {
                latent_out[(size_t) t * 64 + c] = raw[(size_t) c * tile_T + (t - left)];
            }
        }
    }
