...
try {
  ...
  assert.ok(await evaluate("document.body.innerText.includes('Track description')"));
  await evaluate("document.querySelector('.quick-style').click()");
  await delay(150);
  assert.ok(await evaluate("document.querySelector('.style-tags button') !== null"));
  assert.ok(await evaluate("document.querySelector('.brief-column textarea').value.length > 0"));
  await evaluate("document.querySelector('.desk-nav a[href=\"#lyrics-editor\"]').click()");
  await delay(100);
  assert.ok(await evaluate("document.getElementById('lyrics-editor').open"));
  await evaluate("document.getElementById('lyrics-editor').open = false; document.querySelector('.center-scroll').scrollTo({top:0,behavior:'instant'})");
  await evaluate("document.querySelectorAll('.engine-mode button')[1].click()");
  await delay(100);
  await evaluate(`{const input=document.querySelector('#score-sampling input[type=range]');input.value='0.83';input.dispatchEvent(new Event('input',{bubbles:true}));}`);
  await delay(100);
  assert.equal(await evaluate("document.querySelector('#score-sampling input[type=number]').value"),'0.83');
  await evaluate("document.querySelector('.generate-btn').click()");
  await delay(350);
  assert.equal(submitted.at(-1).abc_sampling.temperature,.83);
  ...
  await send('Page.navigate', {url:'http://127.0.0.1:18788'});
  await delay(1000);
  await evaluate("document.querySelector('.recent-select').click()");
  await delay(1300);
  assert.ok(await evaluate("document.querySelector('.preview-time').textContent.includes('0:01')"),'preview playback advances');
  await evaluate("document.querySelectorAll('.recent-select')[1].click()");
  await delay(1300);
  assert.ok(await evaluate("document.querySelector('.preview-time').textContent.includes('0:01')"),'switching tracks starts playback after decoding');
  await evaluate("document.querySelector('.preview-play').click();document.querySelectorAll('.track-action')[2].click()");
  await delay(200);
  assert.equal(await evaluate("document.querySelectorAll('.track-action')[2].getAttribute('aria-pressed')"),'true');
  await send('Page.navigate', {url:'http://127.0.0.1:18788'});
  await delay(800);
  assert.equal(await evaluate("document.querySelectorAll('.track-action')[2].getAttribute('aria-pressed')"),'true','favourite persists');
  for (const [width,height,name] of [[1672,941,'reference-desktop'],[1280,800,'reference-laptop'],[390,844,'reference-mobile']]) {
    await send('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:width<500});
    await evaluate("window.scrollTo(0,0);document.querySelector('.center-scroll').scrollTo({top:0,behavior:'instant'})");
    await delay(350);
    assert.ok(await evaluate('document.documentElement.scrollWidth <= innerWidth'),name+' page overflow');
    console.log(name,await evaluate(`JSON.stringify([...document.querySelectorAll('.sound-card')].map(e=>({width:e.clientWidth,scroll:e.scrollWidth})))`));
    const capture=await send('Page.captureScreenshot',{format:'png'});
    fs.writeFileSync(path.join(output,name+'.png'),Buffer.from(capture.data,'base64'));
  }
  assert.deepEqual(errors,[]);
  console.log('PASS: style selection, lyrics navigation, manual slider submission, automatic generation settings, desktop/laptop/mobile layout; no runtime exceptions.');
} finally {
